...
export default function TodoList() {
  ...
  useEffect(()=>{
    const fetchData= ()=>{
      setLoading(true)
      fire.collection("todos").onSnapshot((snapshot)=>{
  ...
  const addTodo = async ()=>{
    ...
    await fire.collection("todos").add({
      todoText:newTodo,
      checked:false
    }).catch(error=>{
      console.log(error.message)
  ...
  const deletTodo = async (idOfTodo)=>{
    await fire.collection('todos').doc(idOfTodo).delete()
    .catch(error=>{
      console.log(error.message)
    })
  }
  ...
  const handleToggle = async (idOfTodo) => {
    await fire.collection('todos').doc(idOfTodo).update({
  ...
  return (
    ...
    {(todos.length<1 && !loading)?<h2 className="text-center">what you're gonna write , it show here</h2>:
